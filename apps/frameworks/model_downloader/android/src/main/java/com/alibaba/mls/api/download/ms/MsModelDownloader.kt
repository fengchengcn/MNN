...
class MsModelDownloader(override var callback: ModelRepoDownloadCallback?, cacheRootPath: String) :
        ModelRepoDownloader() {
    companion object {
     ...
     * @param calculateSize whether to calculate repo size (not needed for ModelScope as size is
     * included)
    ...
    private suspend fun fetchRepoInfo(modelId: String, calculateSize: Boolean = false): MsRepoInfo {
        ...
        return withContext(DownloadCoroutineManager.downloadDispatcher) {
            runCatching {
                ...
                val split =
                        msModelId.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                if (split.size != 2) {
                    throw FileDownloadException(
                            "Invalid model ID format for $modelId, expected format: owner/repo"
                    )
                }
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    .getOrElse { exception ->
                        ...
                        throw FileDownloadException(
                                "Failed to fetch repo info for $modelId: ${exception.message}"
                        )
                    }
        }
    }
    ...
    override fun download(modelId: String) {
        ...
        DownloadCoroutineManager.launchDownload { downloadMsRepo(modelId) }
    }
            ...
            runCatching {
                ...
                repoInfo.Data?.Files?.filter { it.Type != "tree" }?.sumOf { it.Size } ?: 0L
                    ...
                    .getOrElse { exception ->
                        ...
                        try {
                            val marketSize =
                                    com.alibaba.mls.api.download.DownloadPersistentData
                                            .getMarketSizeTotal(ApplicationProvider.get(), modelId)
                            if (marketSize > 0) {
                                Log.d(
                                        TAG,
                                        "Using saved market size as fallback for $modelId: $marketSize"
                                )
    ...
    private suspend fun downloadMsRepo(modelId: String) {
        ...
        try {
            ...
            withContext(Dispatchers.IO) { downloadMsRepoInner(modelId, modelScopeId, repoInfo) }
    ...
    private fun downloadMsRepoInner(modelId: String, modelScopeId: String, msRepoInfo: MsRepoInfo) {
        ...
        val folderLinkFile = File(cacheRootPath, getLastFileName(modelScopeId))
        if (folderLinkFile.exists() && isValidModelDir(folderLinkFile)) {
            Log.d(TAG, "downloadMsRepoInner already exists and is valid")
            callback?.onDownloadFileFinished(modelId, folderLinkFile.absolutePath)
            return
        }

        // If it exists but is invalid (e.g. empty from failed previous copy), delete it
        if (folderLinkFile.exists()) {
            Log.w(TAG, "Existing model dir $folderLinkFile is invalid/empty, deleting to retry")
            if (folderLinkFile.isDirectory) {
                deleteDirectoryRecursively(folderLinkFile)
            } else {
                folderLinkFile.delete()
            }
        }
                ...
                object : FileDownloadListener {
                    override fun onDownloadDelta(
                        ...
                        callback?.onDownloadingProgress(
                                ...
                                "file",
                                fileName,
                                totalAndDownloadSize[1],
                                totalAndDownloadSize[0]
                        )
        ...
        if (!hasError) {
            ...
            Log.e(TAG, "Errors occurred during download: $errorInfo")
        }
    }
    ...
    private fun collectMsTaskList(
        ...
        for (i in msRepoInfo.Data?.Files?.indices!!) {
            ...
            fileDownloadTask.fileMetadata!!.location =
                    String.format(
            ...
            fileDownloadTask.downloadedSize =
                    if (fileDownloadTask.blobPath!!.exists()) fileDownloadTask.blobPath!!.length()
                    else
                            (if (fileDownloadTask.blobPathIncomplete!!.exists())
                                    fileDownloadTask.blobPathIncomplete!!.length()
                            else 0)
    ...
    private fun isValidModelDir(dir: File): Boolean {
        ...
        // Expanded essential files check
        val essentialFiles =
                arrayOf(
                        "config.json",
                        "llm.mnn",
