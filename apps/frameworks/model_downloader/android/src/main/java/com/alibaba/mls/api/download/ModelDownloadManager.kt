...
/**
 * Minimal ModelDownloadManager for framework Apps can extend or wrap this class for additional
 * functionality
 */
class ModelDownloadManager(context: Context) : ModelRepoDownloader.ModelRepoDownloadCallback {
    ...
    companion object {
        ...
        fun getInstance(context: Context): ModelDownloadManager {
            return instance
                    ?: synchronized(this) {
                        instance
                                ?: ModelDownloadManager(context.applicationContext).also {
                                    instance = it
                                }
                    }
        }
    ...
    var cacheDir: String = sCacheDir ?: (context.filesDir.absolutePath + "/.mnnmodels")
        private set
    private val hfDownloader: HfModelDownloader
    private val msDownloader: MsModelDownloader

    private var downloadListener: DownloadListener? = null
    private val downloadListeners: MutableList<DownloadListener> =
    ...
    fun startDownload(modelId: String) {
        ...
        info.progress = 0.0
        Log.d("ModelDownloadManager", "startDownload: $modelId, cacheDir: $cacheDir")
    ...
    fun getDownloadInfo(modelId: String): DownloadInfo {
        ...
        val info =
                if (isDownloaded) {
                    val totalSize =
                            DownloadPersistentData.getDownloadSizeTotal(
                                    ApplicationProvider.get(),
                                    modelId
                            )
                    DownloadInfo(
                            downloadState = DownloadState.DOWNLOAD_SUCCESS,
                            downlodaState = DownloadState.DOWNLOAD_SUCCESS,
                            progress = 1.0,
                            totalSize = totalSize
                    )
                } else {
                    ...
                    if (totalSize > 0) {
                        ...
                        val state =
                                if (savedSize > 0) DownloadState.PAUSED else DownloadState.NOT_START
                        DownloadInfo(
                                downloadState = state,
                                downlodaState = state,
                                progress = progress,
                        ...
                        DownloadInfo(
                                ...
                                downlodaState = DownloadState.NOT_START
                        )
                    }
                }
    ...
    override fun onDownloadFileFinished(modelId: String, absolutePath: String) {
        ...
        if (info.totalSize > 0) {
            DownloadPersistentData.saveDownloadSizeTotal(
                    ApplicationProvider.get(),
                    modelId,
                    info.totalSize
            )
    ...
    override fun onDownloadingProgress(
        ...
        if (currentTime - tracker.lastTime >= 1000) {
            ...
            if (timeDelta > 0 && bytesDelta >= 0) {
                ...
                val speedStr =
                        if (speed < 1024) "$speed B"
                        else if (speed < 1024 * 1024) String.format("%.2f KB", speed / 1024.0)
                        else String.format("%.2f MB", speed / (1024.0 * 1024.0))
        ...
        Log.d(
                ...
                "onDownloadingProgress: $modelId, progress: $progress, saved: $saved, total: $total"
        )
    ...
    fun getRealDownloadSize(modelId: String): Long {
        val savedSize =
                DownloadPersistentData.getDownloadSizeSaved(ApplicationProvider.get(), modelId)
